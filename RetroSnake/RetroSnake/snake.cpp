...
int CreateSnake(int m, int n, point snake[5][100], int len[5], int direct[5])	//生成初始蛇
{
	...
	setcolor(15);	//第0条蛇白色
	snake[0][1].x = n; snake[0][1].y = m - 6;//第0条蛇board[n][m-6]+board[n][m-4]+board[n][m-2], 白色
	...
	setcolor(13);	//第1条蛇粉色
	snake[1][1].x = 1; snake[1][1].y = 6;  //第1条蛇board[1][6]+board[1][4]+board[1][2]
...
int move(int t, point snake[5][100], int len[5], int direct[5], GamePanel& gp, int& iterationtime)
{
	...
	/***********************************************************************/
	/***/ int nextdirection = machine_move(snake, len, direct, t, gp); /***/
	/***********************************************************************/
	...
	bool selfkill = false;
	if (check(snake, len, t, gp, nextdirection) == false) { selfkill = true; temp = op; }//lose
	else
	{
...
int main()
{
	...
	int score = 0;
	bool wallflag = false;
	bool propsflag = false;
	int iterationtime = 0;
	int m, n;
	m = gp.m, n = gp.n;
	point snake[5][100];//1头 len尾 
	int len[5], direct[8];
	...
	while (1)
	{
		...
		while (!_kbhit())
		{
			...
			if (gp.currentfoodnum == 0 && !wallflag)	//第二面开启障碍物
			{
				rand_wall(snake, len, gp);
				wallflag = true;
			}
			if (gp.currentfoodnum == 0 && !propsflag)	//第二面开启道具
			{
